...
public:
    ...
    int64_t GetchipcoinBalance() const;
    ...
    bool SendCHPToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendAnonToCHP(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    int64_t GetchipcoinDebit(const CTxIn& txin) const;
    int64_t GetchipcoinCredit(const CTxOut& txout) const;
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetchipcoinCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nCHP, int64_t& nchipcoin) const
    {
        nCHP = 0;
        nchipcoin = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nchipcoin += GetchipcoinCredit(txout);
            } else
                nCHP += GetCredit(txout);
            if (!MoneyRange(nCHP)
                || !MoneyRange(nchipcoin))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
    };
...
public:
    ...
    mutable bool fAvailablechipcoinCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailablechipcoinCreditCached;
    
    mutable int64_t nCredCHPCached;
    mutable int64_t nCredchipcoinCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailablechipcoinCreditCached = false;
        
        nCredCHPCached = 0;
        nCredchipcoinCached = 0;
        ...
        nAvailablechipcoinCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredCHP, int64_t& nCredchipcoin, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredCHP = nCredchipcoin = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredCHPCached = 0;
            nCredchipcoinCached = 0;
            pwallet->GetCredit(*this, nCredCHPCached, nCredchipcoinCached);
            fCreditSplitCached = true;
        };
        
        nCredCHP = nCredCHPCached;
        nCredchipcoin = nCredchipcoinCached;
        return true;
    }
    ...
    int64_t GetAvailablechipcoinCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailablechipcoinCreditCached)
            return nAvailablechipcoinCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailablechipcoinCredit() : value out of range");
            };
        };
        
        nAvailablechipcoinCreditCached = nCredit;
        fAvailablechipcoinCreditCached = true;
        return nCredit;
    };
