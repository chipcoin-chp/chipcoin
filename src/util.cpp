...
void createConf()       //Automatic  generation
{
	srand(time(NULL));

	ofstream pConf;
	pConf.open(GetConfigFile().generic_string().c_str());
    const char* nodes =                  //List of known nodes, to be periodically updated
		                 "\nrpcallowip=127.0.0.1"
		                 "\ndaemon=1"
		                 "\nserver=1"
		                 "\naddnode=47.52.88.143"	
		                 "\naddnode=seed1.chipcoin.co"
		                 "\naddnode=seed2.chipcoin.co"
		                 "\naddnode=seed3.chipcoin.co"
		                 "\naddnode=seed4.chipcoin.co"
		                 "\naddnode=seed5.chipcoin.co";						 
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override chipcoin.conf
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    LogPrintf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("chipcoin"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING);
                }
            }
        }
        if (fDebug) {
