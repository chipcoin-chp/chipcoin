...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return chipcoinGUI::tr("Received with");
    case RecvFromOther:
        return chipcoinGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return chipcoinGUI::tr("Sent to");
    case SendToSelf:
        return chipcoinGUI::tr("Payment to yourself");
    case Generated:
        return chipcoinGUI::tr("Mined");
    case Recvchipcoin:
        return chipcoinGUI::tr("Received chipcoin");
    case Sendchipcoin:
        return chipcoinGUI::tr("Sent chipcoin");
    case Other:
        return chipcoinGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::Recvchipcoin:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::Sendchipcoin:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredCHP, nCredchipcoin;
    wtx.GetCredit(nCredCHP, nCredchipcoin, true);
    int64_t nCredit = nCredCHP + nCredchipcoin;
    ...
    {
        if (nNet > 0 && nCredchipcoin > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::Recvchipcoin, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::Sendchipcoin, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::Recvchipcoin;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv chipcoin
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::Sendchipcoin;
                    sub.address = CBitcoinAddress(ckidD).ToString();
