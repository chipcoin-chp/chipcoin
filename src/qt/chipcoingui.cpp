// Copyright (c) 2014 The chipcoin developers
...
chipcoinGUI::chipcoinGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new chipcoinBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("chipcoin") + " - " + tr("Client"));
#ifndef Q_OS_MAC
...
#ifdef Q_OS_WIN
    QFile html("C:/chipcoin/index.html");
#else
    QFile html("/opt/chipcoin/index.html");
#endif
...
void chipcoinGUI::pageLoaded(bool ok)
{
...
void chipcoinGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void chipcoinGUI::urlClicked(const QUrl & link)
{
...
void chipcoinGUI::createActions()
{
...
void chipcoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void chipcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("chipcoin") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/chipcoin_testnet"));
            setWindowIcon(QIcon(":icons/chipcoin_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/chipcoin_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("chipcoin client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/chipcoin_testnet"));
...
void chipcoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void chipcoinGUI::setMessageModel(MessageModel *messageModel)
{
...
void chipcoinGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setIcon(QIcon(":/icons/chipcoin"));
...
#ifndef Q_OS_MAC
void chipcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void chipcoinGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to chipcoin network", "", count));
}

void chipcoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void chipcoinGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void chipcoinGUI::changeEvent(QEvent *e)
{
...
void chipcoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void chipcoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::CHP, nFeeRequired));
...
void chipcoinGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void chipcoinGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void chipcoinGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void chipcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void chipcoinGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void chipcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
void chipcoinGUI::handleURI(QString strURI)
{
...
void chipcoinGUI::setEncryptionStatus(int status)
{
	...
	encryptionIcon   .setAttribute("src", "qrc:///icons/lock_closed");
...
void chipcoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void chipcoinGUI::changePassphrase()
{
...
void chipcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void chipcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void chipcoinGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void chipcoinGUI::updateStakingIcon()
{
...
void chipcoinGUI::detectShutdown()
{
