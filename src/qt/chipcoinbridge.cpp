#include "chipcoinbridge.h"

#include "chipcoingui.h"
#include "guiutil.h"
...
#include "chipcoinbridge.moc"

chipcoinBridge::chipcoinBridge(chipcoinGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void chipcoinBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void chipcoinBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void chipcoinBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void chipcoinBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void chipcoinBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void chipcoinBridge::populateOptions()
{
...
// Transactions
bool chipcoinBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void chipcoinBridge::clearRecipients()
{
    recipients.clear();
}

bool chipcoinBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 CHP, 1 chipcoin
        switch(rcp.txnTypeInd)
        {
            case TXT_CHP_TO_CHP:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::CHP, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_CHP_TO_ANON:
                formatted.append(tr("<b>%1</b> to CHP %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::CHP, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> CHP, ring size %2 to CHP %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::CHP, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_CHP:
                formatted.append(tr("<b>%1</b> CHP, ring size %2 to CHP %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::CHP, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::CHP, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedschipcoinBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your chipcoin balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::CHP, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void chipcoinBridge::openCoinControl()
{
...
void chipcoinBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void chipcoinBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap chipcoinBridge::listAnonOutputs()
{
...
void chipcoinBridge::populateTransactionTable()
{
...
void chipcoinBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void chipcoinBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString chipcoinBridge::transactionDetails(QString txid)
{
...
// Addresses
void chipcoinBridge::populateAddressTable()
{
...
void chipcoinBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void chipcoinBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString chipcoinBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString chipcoinBridge::lastAddressError()
{
    QString sError;
...
QString chipcoinBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void chipcoinBridge::updateAddressLabel(QString address, QString label)
{
...
bool chipcoinBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool chipcoinBridge::checkVersion()
{
    ...
    // the HTTP request
    QNetworkRequest req( QUrl( QString("http://walletupdate.chipcoin.org/version.js") ) );
...
bool chipcoinBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void chipcoinBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void chipcoinBridge::appendMessage(int row)
{
...
void chipcoinBridge::populateMessageTable()
{
...
void chipcoinBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool chipcoinBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool chipcoinBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString chipcoinBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString chipcoinBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool chipcoinBridge::setPubKey(QString address, QString pubkey)
{
...
bool chipcoinBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString chipcoinBridge::createGroupChat(QString label)
{
...
QString chipcoinBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList chipcoinBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void chipcoinBridge::connectSignals()
{
...
QString chipcoinBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap chipcoinBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap chipcoinBridge::listLatestBlocks()
{
...
QVariantMap chipcoinBridge::findBlock(QString searchID)
{
...
QVariantMap chipcoinBridge::blockDetails(QString blkHash)
{
...
QVariantMap chipcoinBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap chipcoinBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap chipcoinBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap chipcoinBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap chipcoinBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap chipcoinBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap chipcoinBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap chipcoinBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap chipcoinBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with chipcoin prefix.");
            return result;
        }
...
QVariantMap chipcoinBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap chipcoinBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap chipcoinBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
